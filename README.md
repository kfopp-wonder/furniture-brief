...
8. **Substack drafts without the Mac** (optional). Substack's Cloudflare rules challenge GitHub's IP range (a plain client, a Chrome-fingerprint client and a real headless Chromium all get the "Just a moment" page), so creating the draft from GitHub needs a residential IP. Buy one static residential proxy from any provider (Webshare, IPRoyal, Bright Data and similar sell single IPs for a few dollars a month), add it as the `SUBSTACK_PROXY` secret in the form `http://user:pass@host:port`, keep `SUBSTACK_SID` set, and the daily run creates the draft itself; the review email then carries an **Open draft in Substack** button and the Mac job stands down automatically. Verify with Actions → Maintenance → `check-substack` (it prints the egress IP and the four API probes). The same proxy is used to resolve beehiiv newsletter links, which sit behind the same kind of block.
9. **Substack drafts from your Mac** (alternative, free). Run once in Terminal:
